...
G4ThreadLocal G4Allocator<ECalECHit>* ECalECHitAllocator;
...
ECalECHit::ECalECHit(G4int cellID)
: G4VHit(), 
...
ECalECHit::ECalECHit(const ECalECHit &right)
: G4VHit(),
...
const ECalECHit& ECalECHit::operator=(const ECalECHit &right)
{
  fCellID = right.fCellID;
  fEdep = right.fEdep;
  fPos = right.fPos;
...
int ECalECHit::operator==(const ECalECHit &right) const
{
...
const std::map<G4String,G4AttDef>* ECalECHit::GetAttDefs() const
{
  G4bool isNew;
  auto store = G4AttDefStore::GetInstance("ECalECHit",isNew);
...
std::vector<G4AttValue>* ECalECHit::CreateAttValues() const
{
  auto values = new std::vector<G4AttValue>;
  
  values
    ->push_back(G4AttValue("HitType","ECalECHit",""));
  values
