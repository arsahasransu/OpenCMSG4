#include"HadronicProcessHelper.hh"
...
HadronicProcessHelper* HadronicProcessHelper::s_instance = 0;

HadronicProcessHelper* HadronicProcessHelper::instance() 
{
  if (s_instance == 0)
    {  
      s_instance = new HadronicProcessHelper();
    }
  return s_instance; 
}
...
//  const G4DynamicParticle* incidentDynamicParticle = track.GetDynamicParticle();
  ...
  while(!selected && tries < 100){
    ...
    if(twoToThreeFlag[i]) {
      ...
      if (m_phaseSpace(goodReactionProductList[i],incidentDynamicParticle,target)>CLHEP::RandFlat::shoot()) selected = true;
    } else {
