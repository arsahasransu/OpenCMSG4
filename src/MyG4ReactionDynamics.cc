...
*/
 ...
 G4bool MyG4ReactionDynamics::GenerateXandPt(
...
//      G4ReactionProduct *pVec = new G4ReactionProduct [nuclearExcitationCount];
      ...
      if( vec[i]->GetNewlyAdded() )           // added from intranuclear cascade
      {
        if( vec[i]->GetSide() == -2 )         //  is a nucleon
        {
          if( backwardNucleonCount < 18 )
          {
            ...
            {
              for(G4int ctr=0; ctr<vecLen; ctr++) delete vec[ctr];
   	      vecLen = 0;
              throw G4HadReentrentException(__FILE__, __LINE__,
	      "MyG4ReactionDynamics::GenerateXandPt : a pion has been counted as a backward nucleon");
            }
            vec[i]->SetSide( -3 );
        ...
        if( resetEnergies )
        {
          ...
          for( l=i+1; l<vecLen; ++l )
          {
                ...
                vec[l]->GetDefinition() == aPiPlus )
            {
              ...
              if( pp1 < 1.0e-6*GeV )
              {
                G4double rthnv = pi*G4UniformRand();
                G4double phinv = twopi*G4UniformRand();
                G4double srth = std::sin(rthnv);
                vec[l]->SetMomentum( pp*srth*std::cos(phinv)*MeV,
                                     pp*srth*std::sin(phinv)*MeV,
                                     pp*std::cos(rthnv)*MeV ) ;
              } else {
    ...
    if( pp1 < 1.0e-6*GeV )
    {
      ...
      G4double phinv = twopi*G4UniformRand();
      G4double srth = std::sin(rthnv);
      currentParticle.SetMomentum( pp*srth*std::cos(phinv)*MeV,
                                   pp*srth*std::sin(phinv)*MeV,
                                   pp*std::cos(rthnv)*MeV );
    } else {
        ...
        while( ++innerCounter < 7 )    // start of inner iteration loop
        {
          ...
          else                    // target has gone to forward side
          {
            ...
            if( pp1 < 1.0e-6*GeV )
            {
              ...
              G4double phinv = twopi*G4UniformRand();
              G4double srth = std::sin(rthnv);
              targetParticle.SetMomentum( pp*srth*std::cos(phinv)*MeV,
                                          pp*srth*std::sin(phinv)*MeV,
                                          pp*std::cos(rthnv)*MeV );
            }
            else
...
//    G4double ekin0 = pseudoParticle[3].GetKineticEnergy()/GeV;
    ...
    G4ParticleDefinition* aOrgDef = (G4ParticleDefinition*)modifiedOriginal.GetDefinition();
    G4int diff = 0;
    if(aOrgDef == G4Proton::Proton() || aOrgDef == G4Neutron::Neutron() )  diff = 1;
    //if(modifiedOriginal.GetDefinition() == G4Proton::Proton() || modifiedOriginal.GetDefinition() == G4Neutron::Neutron() )  diff = 1;
...
//       G4cout << "JPW ### "<<jpw<<G4endl;
 ...
 void MyG4ReactionDynamics::SuppressChargedPions(
   G4FastVector<G4ReactionProduct,MYGHADLISTSIZE> &vec,
   G4int &vecLen,
 ...
 G4bool MyG4ReactionDynamics::TwoCluster(
   G4FastVector<G4ReactionProduct,MYGHADLISTSIZE> &vec,
   G4int &vecLen,
   G4ReactionProduct &modifiedOriginal, // Fermi motion & evap. effects included
   const G4HadProjectile *originalIncident, // the original incident particle
   ...
   G4ReactionProduct &leadingStrangeParticle )
  {
	...
	vecLen = 0;
        throw G4HadReentrentException(__FILE__, __LINE__,
	"MyG4ReactionDynamics::TwoCluster: Negative number of particles");
      }
      delete vec[vecLen-1];
      delete vec[vecLen-2];
      vecLen -= 2;
...
//    G4double ekin0 = pseudoParticle[4].GetKineticEnergy()/GeV;
 ...
 void MyG4ReactionDynamics::TwoBody(
  G4FastVector<G4ReactionProduct,MYGHADLISTSIZE> &vec,
  G4int &vecLen,
...
//      }
      ...
      */
      ...
      if( pf <= 0.)// 0.001 )
      {
        ...
        vecLen = 0;
	throw G4HadronicException(__FILE__, __LINE__, "MyG4ReactionDynamics::TwoBody: pf is too small ");
      }
 ...
 G4double MyG4ReactionDynamics::GenerateNBodyEvent(
...
//      // DEBUGGING --> DumpFrames::DumpFrame(vec, vecLen);
    ...
    if( vecLen < 2 )
    {
      G4cerr << "*** Error in MyG4ReactionDynamics::GenerateNBodyEvent" << G4endl;
    ...
    G4double bang, cb, sb, s0, s1, s2, c, sc, esys, a, b, gama, beta;
    pcm[0][0] = 0.0;
    pcm[1][0] = pd[0];
    pcm[2][0] = 0.0;
    for( i=1; i<vecLen; ++i )
 ...
 G4int
   MyG4ReactionDynamics::Poisson( G4double x )  // generation of poisson distribution
   {
     G4int iran;
     G4double ran;
     ...
     if( x > 9.9 )    // use normal distribution with sigma^2 = <x>
      ...
      G4int mmx = G4int(5.0*x);
      if( mmx <= 0 )   // for very small x try iran=1,2,3
      {
 ...
 G4int
   MyG4ReactionDynamics::Factorial( G4int n )
   {   // calculates factorial( n ) = n*(n-1)*(n-2)*...*1
     G4int mi = std::min(n,10);
     G4int result = 1;
     if( mi <= 1 )return result;
     for( G4int i=2; i<=mi; ++i )result *= i;
     return result;
   }
 
 void MyG4ReactionDynamics::Defs1(
   const G4ReactionProduct &modifiedOriginal,
   G4ReactionProduct &currentParticle,
   G4ReactionProduct &targetParticle,
   G4FastVector<G4ReactionProduct,MYGHADLISTSIZE> &vec,
 ...
 void MyG4ReactionDynamics::Rotate(
  ...
  G4int &vecLen )
  {
    ...
    G4ThreeVector fermion(ran1, ran2, 0);

    pseudoParticle[0] = pseudoParticle[0]+fermion; // all particles + fermion
 ...
 void MyG4ReactionDynamics::AddBlackTrackParticles(
 ...
 void MyG4ReactionDynamics::MomentumCheck(
   const G4ReactionProduct &modifiedOriginal,
   G4ReactionProduct &currentParticle,
   G4ReactionProduct &targetParticle,
   G4FastVector<G4ReactionProduct,MYGHADLISTSIZE> &vec,
 ...
 void MyG4ReactionDynamics::ProduceStrangeParticlePairs(
   G4FastVector<G4ReactionProduct,MYGHADLISTSIZE> &vec,
   G4int &vecLen,
   const G4ReactionProduct &modifiedOriginal,
   const G4DynamicParticle *originalTarget, 
   ...
   G4bool &targetHasChanged )
  {
    ...
    {
      G4double randomvar = G4UniformRand();
      while( randomvar == 1.0 )randomvar = G4UniformRand();
      i4 = i3 = G4int( vecLen*randomvar );
      while( i3 == i4 )
      {
        randomvar = G4UniformRand();
        while( randomvar == 1.0 )randomvar = G4UniformRand();
        i4 = G4int( vecLen*randomvar );
      }
    }
    //
 ...
 void
  MyG4ReactionDynamics::NuclearReaction(
   ...
   const G4double *mass )
  {
    ...
    if( index == 9 )  // loop continued to the end
    {
      throw G4HadronicException(__FILE__, __LINE__,
           "MyG4ReactionDynamics::NuclearReaction: inelastic reaction kinematically not possible");
    }
